...
impl Agent for Substation {
    fn process_tick(&mut self, setpoint: f64, inertia: f64) -> f64 {
        ...
        let phase_correction = (setpoint - impact) * self.learning_rate * 0.15;
...
fn draw_dashboard(
    ...
    print!("\x1B[J");
    println!("=== GRID STABILITY MONITOR ===");
    println!(
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut state = GridState {
        base_setpoint: sim_config.initial_setpoint,
        inertia: sim_config.initial_inertia,
        ..Default::default()
    };

    let checkpoint_file = "examples/grid_checkpoint.bin";
    if let Ok(loaded) = BinarySerializer::load::<GridState>(checkpoint_file) {
    ...
    println!("[!] Grid Stability Domain started.");
    ...
    loop {
        ...
        let mut current_setpoint = state.base_setpoint + daily_cycle;
        ...
        let impacts = manager.process_all_agents_parallel(current_setpoint, state.inertia);
        ...
        while manager.needs_homeostasis() && manager.len() < sim_config.max_population {
            ...
            manager.add_agent(Box::new(Substation::new(
                &format!("SUB-{}", state.total_spawned),
                state.tick as u32,
            )));
        }
